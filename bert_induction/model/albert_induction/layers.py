...
def induction_with_DR(sample_prediction_vector,
                      iter_routing=3,
                      ):
    """
    使用动态路由算法获取类向量
    Args:
        sample_prediction_vector: float tensor  [batch_size, c, k, attention_size]
        iter_routing: 迭代次数

    Returns:
        class vector. float tensor [batch_size, c, attention_size]
    """
    batch_size, c, k, attention_size = get_shape_list(sample_prediction_vector, expected_rank=4)
    b = tf.constant(np.zeros([batch_size, c, 1, k], dtype=np.float32), dtype=tf.float32)
    e = sample_prediction_vector  # [batch_size, c, k, attention_size]
    ...
    for r_iter in range(iter_routing):
        with tf.variable_scope("iter_" + str(iter_routing)):
            ...
            if r_iter < iter_routing - 1:  # 前面的迭代，不需要梯度下降
                c_hat_stopped = tf.matmul(d, e_hat_stopped)  # [batch_size, c, 1, attention_size]
                c = squash(c_hat_stopped)  # [batch_size, c, 1, attention_size]
                # 更新b
                # b = b + e_hat * c
                c_expend = tf.tile(c, [1, 1, k, 1])  # [batch_size, c, k, attention_size]
                ...
                c_hat = tf.matmul(d, e_hat)  # [batch_size, c, 1, attention_size]
                c = squash(c_hat)  # [batch_size, c, 1, attention_size]
                c = tf.squeeze(c, [2])  # [batch_size, c, attention_size]
                # 不需要更新b
    return c
...
def relation_model(class_vector,
    ...
    Args:
        class_vector: [batch_size, c, attention_size]
        query_input: [batch_size, query_size, attention_size]
        h: NTN size

    Returns:
        relation score. [batch_size, query_size, c]
    """
    batch_size, c, attention_size = get_shape_list(class_vector, expected_rank=3)
    _, query_size, _ = get_shape_list(query_input, expected_rank=3)
    class_vector = tf.expand_dims(class_vector, 1)
    class_vector = tf.tile(class_vector, [1, query_size, 1, 1])  # [batch_size, query_size, c, attention_size]
    g = []
    for i in range(h):
        transformed = tf.layers.dense(query_input, attention_size, use_bias=False,
                                      kernel_initializer=initializers.get(
                                          "glorot_uniform"))  # [batch_size, query_size, attention_size]
        transformed = tf.expand_dims(transformed, 2)  # [batch_size, query_size, 1, attention_size]
        transformed = tf.tile(transformed, [1, 1, c, 1])  # [batch_size, query_size, c, attention_size]
        g.append(gelu(tf.reduce_sum(tf.multiply(transformed, class_vector), axis=-1,
                                    keepdims=False)))  # [batch_size, query_size, c]
    relation_vector = tf.stack(g, axis=-1)  # [batch_size, query_size, c, h]
    score = tf.layers.dense(relation_vector, 1, activation=tf.nn.sigmoid, name="score")
    return tf.squeeze(score, axis=-1)  # [batch_size, query_size, c]


def relation_model_MANL(class_vector, support_inputs, learning_rate,
